...
class EmbeddingService:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            if embedding_dim != settings.vector.vector_size:
                logger.warning(f"Model dimension ({embedding_dim}) != configured dimension ({settings.vector.vector_size})")
    ...
    def generate_embedding(self, text: str) -> Tuple[List[float], float]:
        ...
        try:
            ...
            logger.debug(f"Generated embedding for text ({len(text)} chars) in {generation_time:.3f}s")
    ...
    def generate_embeddings_batch(self, texts: List[str]) -> Tuple[List[List[float]], float]:
        ...
        try:
            ...
            logger.info(f"Average time per embedding: {generation_time/len(all_embeddings):.3f}s")
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "max_sequence_length": getattr(self.model.tokenizer, 'model_max_length', 'Unknown'),
